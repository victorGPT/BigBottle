...
describe('ScanPage', () => {
  ...
  it('shows reward rules before receipt capture', () => {
    ...
    expect(
      screen.getByText(
        'Voters or GM-NFT holders: 1 successful chance per day within each week, with base receipt points capped at 2 before the 100x multiplier.'
      )
    ).toBeInTheDocument();
  ...
  it('retries upload with acl header when first put returns 403', async () => {
    ...
    mocks.fetch
      .mockResolvedValueOnce({ ok: false, status: 403, text: async () => '<Code>AccessDenied</Code>' })
    ...
    expect((mocks.fetch.mock.calls[1]?.[1] as any)?.headers).toMatchObject({
