-- BigBottle: cap users without vote/GM-NFT bonus privileges to one upload per UTC week.
...
begin
  ...
  select exists (
    ...
    where s.user_id = new.user_id
      and exists (
        ...
        where source->>'type' = 'gm_nft'
      )
  ) into has_bonus_privileges;

  if has_bonus_privileges then
  ...
  if tg_op = 'INSERT' then
    ...
    if has_bonus_privileges and upload_count >= 2 then
      raise exception using
        errcode = 'P0001',
        message = 'daily_upload_limit_exceeded';
    end if;

    if not has_bonus_privileges and upload_count >= 1 then
      raise exception using
        errcode = 'P0001',
  ...
  if new.status = 'verified' and (tg_op = 'INSERT' or old.status is distinct from new.status) then
    ...
    if has_bonus_privileges and verified_count >= 1 then
      raise exception using
        errcode = 'P0001',
        message = 'daily_verified_limit_exceeded';
    end if;

    if not has_bonus_privileges and verified_count >= 2 then
      raise exception using
        errcode = 'P0001',
